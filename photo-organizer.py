...
FILE_TYPES_TO_PROCESS = ('.jpg', '.jpeg', '.mov', '.avi',
                         '.mp4', '.cr2', '.3gp', '.wmv', '.mpg', '.m4v', '.png')
FILES_TO_DELETE = ('.picasa.ini', 'picasa.ini', '.ds_store',
                   '._.ds_store', 'thumbs.db', 'zbthumbnail.info')
DATETIME_EXIF_TAG = 'Image DateTime'
DATETIME_EXIF_TAG_FORMAT = '%Y:%m:%d %H:%M:%S'
FILE_DUP_MAX_RETRIES = 50

RE_DATE = re.compile(r'.*?\D*(20\d{6})\D.*')
...
def get_file_date(path):
    ...
    if DATETIME_EXIF_TAG in tags:
        ...
        try:
            ...
            print(e)
    m = RE_DATE.match(file_name)
    if m:
        return datetime.strptime(m.groups()[0], '%Y%m%d')
    else:
...
if __name__ == "__main__":
    ...
    for f in get_files(src):
        ...
        if not d:
            ...
            continue
        dest_folder = os.path.join(dest, "{0}-{1:02}".format(d.year, d.month))
        ...
        if os.path.exists(dest_path):
            if os.stat(f).st_size == os.stat(dest_path).st_size and \
                    md5(f) == md5(dest_path):
                print("skipping {0}, already exists as {1} with same size and hash".format(
                    f, dest_path))
                if not args.copy_only:
                    os.remove(f)
                continue
                ...
                while os.path.exists(new_path) and counter < FILE_DUP_MAX_RETRIES:
                    new_path = ("-" + str(counter)
                                ).join(os.path.splitext(dest_path))
                    counter += 1
                if counter < FILE_DUP_MAX_RETRIES:
                    dest_path = new_path
                else:
                    print("error: cannot copy {0}, already exists as {1}".format(
                        f, new_path))
